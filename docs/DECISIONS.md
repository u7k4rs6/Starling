...
## 0031 - Deploy, Phase 3: a per-boot generation token, so a relay restart cannot silently split a room
...
The token is per boot, which covers the restart case, the one that actually happens on the free host. It does not cover a doc evicted from the in-memory cache while the process keeps running (the LRU at `MAX_DOCS`), whose token would not change: a client returning to an evicted-then-recreated room could still split. That is why this phase does not add time-based idle eviction even though earlier notes floated it: idle eviction plus a per-boot token would manufacture exactly this silent-split path. The doc-count cap alone bounds memory (see #0032), so nothing is lost by leaving eviction to that cap, and reaching it takes 128 simultaneously-active rooms, which a portfolio demo does not see. A per-document token would close the eviction case too and is the natural extension if this ever outgrows a demo.
