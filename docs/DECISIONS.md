...
- *2 MB is about 170,000 typed ops.* Measured by `bench/op-size.mjs` (`pnpm run bench:op-size`) across workloads, not just the favourable one: a sequential insert at the end encodes to 12.3 bytes, an interior insert also to 12.3, mixed edit churn to 10.6, and inserts with 50% deletes to 8.2 (a delete/tombstone op is *smaller* than an insert, since it carries an id but no character or origin). So 12.3 bytes/op is the conservative figure for inserts, not an understatement, and a 2 MB log holds roughly 170,000 insert ops. The log is append-only and never compacted, so that is cumulative over a room's whole life. By typing alone that is on the order of five hours of continuous keystrokes; a demo session is minutes and a few thousand ops, and a room is spun down or evicted long before. (This measures `Doc`; `bench/encode-decode.mjs` reports a larger per-op size for `RgaDoc`, which puts different things in each op. Different structures, both honest.)
...
## 0036 - Deploy, Phase 4: the hidden-tab hard stop never resumed, so a cold-start share killed sync

The first live share on the deployed demo failed in a way no test had caught: the cold start worked (the relay woke, the room was created), but sync stopped after the handover. Four relay requests in seven minutes, replica A stuck with unpushed edits, replica B empty, and the strip reading "diverging" with no link cut.

Root cause was the hidden-tab hard stop from Phase 2, not the handover. The demo's sync loop reschedules itself with `setTimeout` and, when `nextSyncDecision` says stop, simply does not set the next timer. The resume, on `visibilitychange` back to visible, was guarded by `if (timer === undefined)`. But `timer` still holds the id of the `setTimeout` that just fired to run the stopping tick, so it is never `undefined` at that point, and the guard never passed. Once a hidden tab crossed the 2-minute grace and stopped, it stayed stopped for good. Production reached it because a real wake is 50 to 60 seconds and a visitor backgrounds the tab while they wait; the tab is still hidden through the handover, hits the 2-minute stop, and never comes back. The 7.6-second rigged cold-start test never backgrounded the tab and never crossed the grace, so the loop never stopped, which is exactly why local testing missed it.

The fix replaces the overloaded `timer` variable with an explicit `stopped` flag: the stop branch sets `stopped = true`, and `visibilitychange` resumes only when `stopped`, which is a state that cannot be confused with a fired-but-not-cleared timer. The resume deliberately does nothing to a still-running loop, so it cannot fire a second tick over one that is mid-await. `switchTransport` was ruled out as the cause: it acts on the Provider, while the loop is independent and keeps calling `sync()` through whatever transport is current; it never stops the scheduler.

The regression is covered end to end. `nextSyncDecision` grew optional timing overrides so an e2e can shrink the 2-minute grace to seconds, and a test backgrounds the tab past the stop, foregrounds it, types, and asserts the edit reaches the other replica. It fails against the pre-fix loop (the edit never propagates) and passes against the fix, which is the reproduction the live bug needed. Separately, the diverged strip copy was corrected: it read "will reconcile once the links are clear", which blames a partition that is not there in this state; it now reads "they will reconcile as edits sync".

## 0037 - Deploy, Phase 5: the harness controlled the dimension the bug was not in

The cold-start stall (#0036) is worth a second entry, not for the fix but for why it shipped. The rigged cold-start test modelled one production dimension carefully, the relay's wake delay, and left another, tab visibility, at its default. The bug lived entirely in the dimension that was not controlled: a stopped hidden-tab loop that never resumed. So 376 passing tests and a green cold-start simulation all agreed the demo worked, because every one of them ran with the tab visible, and the fault was only reachable while hidden. A simulation is only evidence about the axes it varies; holding an axis at its default is not the same as testing it, and it reads as coverage when it is a blind spot. The fix that mattered most was not the `stopped` flag, it was adding visibility as a controlled axis to the harness (the regression test drives `visibilityState` directly), so that dimension is now modelled rather than assumed.

Recording this because the same shape will recur: the local harness still does not model several production dimensions, and the next escaped bug will most likely live in one of them. What is not modelled today:

- **Real network behaviour.** Tests use an in-process relay or a fixed artificial delay. Real TLS handshakes, DNS, variable latency and jitter, mid-flight connection drops, and proxy or CDN quirks are not exercised. The `ControllableTransport` degradations are a deliberate model at the transport seam, not the wire.
- **Real cold-start duration and its variance.** The wake was simulated as a fixed few-second delay; the real 50 to 60 seconds, and its variance, only exist on the deployed instance.
- **Real devices and radios.** Only an emulated mobile viewport ran locally; an actual phone on a cellular connection, with its own timers, backgrounding, and radio wake behaviour, is unmodelled (DEPLOY.md leaves it as a live check).
- **Browser lifecycle beyond visibilitychange.** bfcache restore, tab freezing and discard under memory pressure, and OS-level suspend can pause or tear down timers in ways a dispatched `visibilitychange` does not reproduce.
- **One engine, unpinned.** The e2e runs a single system Chrome whose version floats (#0033); other engines and versions are untested.
- **Wall-clock and budget effects.** The 2-minute hidden grace, the 15-minute spin-down, and the 750 instance-hour budget are reasoned arithmetic (#0029, #0032), run in tests only at shrunken scale, never over real elapsed time.
- **Independent clients across the network.** The demo's two panes share one browser and the integration tests share one process; genuinely separate browsers on separate networks converging through the hosted relay is a shape only the live deployment covers.

## 0038 - New UI: the Starling v8 design, wired to the real engine (not the mockup's sim)

The demo's interface was replaced with the Starling v8 design from the Claude Design canvas: a dark, gold-on-near-black "network instrument" with a light theme, two textarea replicas either side of a hex CORE cut-control, star-encoded break-it sliders, a connection-health strip, a live activity log, and metrics. The design shipped as a `.dc.html` mockup with its own simulated CRDT; the integration keeps the look and wires every moving part to the real `Provider` / `ControllableTransport` engine instead. Several choices are worth recording.

**A textarea binding replaces ProseMirror.** The design uses a plain `<textarea>` with a line-number gutter, so the demo binds the textarea to the Doc directly: on input it diffs the old text against the new value down to the changed span and mints real Fugue ops through `insertLocal` / `deleteLocal` (`text-binding.ts`), and on a remote change it rewrites the textarea and restores the caret through `doc.anchorAt` / `resolveAnchor`. This is simpler than the ProseMirror binding and matches the design. `@starling/editor` (the ProseMirror binding and its undo manager) is no longer used by the demo, though it stays a tested library package; dropping it also cut the bundle from about 400 KB to about 245 KB.

**The mockup's sim was not ported; real state drives the visuals.** The `.dc.html` carried a `DCLogic` component with a fake CRDT, fake op-flight timing, and fake metrics. None of that was copied. Convergence, the pending badges, the DIVERGED / CONVERGED / PARTITIONED / FROZEN verdicts, the header signal bars, and the connection-health star-flow all read the real replicas: real pending counts, real text equality, real cut and frozen state. The activity log records real events (local INSERT/DELETE edits, link CUT/JOIN, and the SYNC-converged moment) and the metrics are real (two replicas, operations in the last 60 seconds, zero conflicts by construction, and a measured average convergence time, the wall-clock gap between the documents diverging and agreeing again). The per-op star flight the mockup animated frame-by-frame is approximated by a state-driven field rather than fabricated, so the wire looks alive only when there is genuinely something in flight.

**Real phase, not preview tabs.** The mockup had a "DESIGN STATES" tab row to preview local, waking, shared, and frozen. Those were a design-review affordance and were dropped; the phase is driven by real behaviour: local by default, waking during the real `GET /health` cold-start probe, shared after the real `switchTransport` handoff, frozen when the real relay log freezes (`isSyncBlocked`). The THEME toggle was kept (dark default, persisted), since a light palette is a real feature the design ships.

**Self-hosted fonts, not the CDN.** The mockup linked JetBrains Mono from Google Fonts. The demo self-hosts it (`@fontsource/jetbrains-mono`, weights 300/400/500/700) to honour SECURITY section 1's no-third-party-requests rule, which the deployed relay's CORS story depends on.

**One regression, noted.** CRDT-aware undo, which the ProseMirror `UndoManager` provided, is gone with ProseMirror. The textarea falls back to the browser's native undo, which is best-effort against concurrent remote edits rather than op-aware. The design did not specify undo, and native undo is acceptable for a demo; a textarea-level CRDT undo could be added later if it earns its place.
